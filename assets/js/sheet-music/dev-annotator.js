...
(function () {
  ...
  const SHARED_DEV_MODE_KEY = "site-dev-mode";

  function setDevModeStorage(enabled) {
    try {
      for (const key of [DEV_MODE_KEY, SHARED_DEV_MODE_KEY]) {
  ...
  function readDevMode() {
    ...
    if (params.get("dev") === "0") {
      setDevModeStorage(false);
      return false;
    }
    try {
      return localStorage.getItem(DEV_MODE_KEY) === "1" || localStorage.getItem(SHARED_DEV_MODE_KEY) === "1";
    } catch (_) {
      return false;
    }
  }
  ...
  function renderDevToggle(devOn) {
    ...
    btn.title = devOn ? "Dev mode is on (annotator + sidebar). Click to turn off." : "Click to enable dev mode (drop pins, sidebar). Or use ?dev=1.";
    btn.addEventListener("click", () => {
      setDevModeStorage(!devOn);
  ...
  // Touch equivalent of shift+click: a brief two-finger tap with little
  // movement drops a pin at the midpoint. Pinch-to-zoom is preserved
  // because any meaningful finger movement (>20px) cancels the gesture.
  // iOS Safari does not synthesize a click for two-finger tap, so we
  // don't clash with the normal single-tap chord-inspector flow.
  const TWO_FINGER_MAX_MS = 500;
  const TWO_FINGER_MAX_MOVE_PX = 20;
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      const t1 = e.touches[0],
        t2 = e.touches[1];
      const d1 = Math.hypot(t1.clientX - twoFingerStart.x1, t1.clientY - twoFingerStart.y1);
      const d2 = Math.hypot(t2.clientX - twoFingerStart.x2, t2.clientY - twoFingerStart.y2);
      if (d1 > TWO_FINGER_MAX_MOVE_PX || d2 > TWO_FINGER_MAX_MOVE_PX) {
        twoFingerStart = null;
      }
    },
    { passive: true }
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      if (e.touches.length > 0) return; // wait until both fingers lift
      const dur = Date.now() - twoFingerStart.t;
      const midClientX = (twoFingerStart.x1 + twoFingerStart.x2) / 2;
      const midClientY = (twoFingerStart.y1 + twoFingerStart.y2) / 2;
      twoFingerStart = null;
      if (dur > TWO_FINGER_MAX_MS) return;
      const target = document.elementFromPoint(midClientX, midClientY);
      if (!target) return;
      if (target.closest && target.closest("[data-sheet-annotator]")) return;
      e.preventDefault();
      const fakeEvt = {
        target,
        pageX: midClientX + window.scrollX,
        pageY: midClientY + window.scrollY,
        shiftKey: true,
        preventDefault() {},
        stopPropagation() {},
      };
  ...
  function classifyTarget(target, pageX, pageY) {
    ...
    if (staveNote) {
      const glyph = target.closest(".vf-notehead, .vf-note") || staveNote.querySelector(".vf-note") || staveNote;
  ...
  async function captureIdentity(ev) {
    ...
    if (chordText) {
      ...
      if (staveNote) {
        const glyph = ev.target.closest(".vf-notehead, .vf-note") || staveNote.querySelector(".vf-note") || staveNote;
        const gr = glyph.getBoundingClientRect();
        resolvedRect = {
    ...
    if (bridge && bridge.ready) {
      try {
        out.thumbnail = await bridge.snapshotAround(ev.pageX, ev.pageY, 220, 140, {
  ...
  async function probeSidecar() {
    ...
    sidebar.setStatus(state.sidecarStatus === "up" ? "sidecar online" : "local only (sidecar offline)", state.sidecarStatus !== "up");
  }
  ...
  function buildSidebar() {
    ...
    root.innerHTML = `
      ...
      <div class="sheet-annotator-sidebar__inner">
        ...
        <p class="sheet-annotator-sidebar__hint" data-hint>Shift-click anywhere on the page to drop a pin.</p>
    ...
    if (hintEl && window.matchMedia && window.matchMedia("(pointer: coarse)").matches) {
      hintEl.textContent = "Two-finger tap on the score to drop a pin.";
    }
    ...
    function downloadJson() {
      const blob = new Blob([JSON.stringify({ pathname: location.pathname, pins: state.pins }, null, 2)], { type: "application/json" });
