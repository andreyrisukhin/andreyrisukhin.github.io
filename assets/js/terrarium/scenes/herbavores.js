...
export function createHerbavoresScene() {
  return {
    ...
    for (const herb of this.herbavores) {
      ...
      if (herb.x < 8) { herb.x = 8; herb.vx *= -0.6; }
      if (herb.x > size.w - 8) { herb.x = size.w - 8; herb.vx *= -0.6; }
      if (herb.y < 8) { herb.y = 8; herb.vy *= -0.6; }
      if (herb.y > size.h - 8) { herb.y = size.h - 8; herb.vy *= -0.6; }
...
export function createHerbavoresSimulation({
  params = {},
  size = { w: 900, h: 520 },
  initial = 12,
  state = null,
