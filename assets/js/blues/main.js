...
(function () {
  ...
  function renderScaleStaff() {
    ...
    var svg = '<svg class="blues-staff-svg" viewBox="0 0 ' + svgWidth + " " + svgHeight + '" xmlns="http://www.w3.org/2000/svg">';

    // 5 staff lines: E4(0), G4(2), B4(4), D5(6), F5(8)
    var staffLineSteps = [0, 2, 4, 6, 8];
    for (var li = 0; li < 5; li++) {
      var ly = bottomLineY - staffLineSteps[li] * (lineSpacing / 2);
      svg += '<line class="blues-staff-line" x1="' + (leftMargin - 5) + '" y1="' + ly + '" x2="' + (svgWidth - rightPad + 5) + '" y2="' + ly + '"/>';
    }

    // Treble clef
    svg +=
      '<text class="blues-staff-clef" x="5" y="' +
    ...
    notes.forEach(function (n, i) {
      ...
      if (n.step < 0) {
        for (var ls = -2; ls >= n.step; ls -= 2) {
          ...
          svg += '<line class="blues-staff-ledger" x1="' + (x - 10) + '" y1="' + lly + '" x2="' + (x + 10) + '" y2="' + lly + '"/>';
        }
      }
      if (n.step > 8) {
        for (var ls2 = 10; ls2 <= n.step; ls2 += 2) {
          var lly2 = bottomLineY - ls2 * (lineSpacing / 2);
          svg += '<line class="blues-staff-ledger" x1="' + (x - 10) + '" y1="' + lly2 + '" x2="' + (x + 10) + '" y2="' + lly2 + '"/>';
        }
      }

      // Accidental
      if (n.acc) {
        svg += '<text class="blues-staff-accidental" x="' + (x - 14) + '" y="' + (y + 4) + '" text-anchor="end">' + n.acc + "</text>";
      }

      // Notehead (tilted ellipse)
      var headClass = n.isBlue ? "blues-staff-notehead blues-staff-notehead--blue" : "blues-staff-notehead";
      svg += '<ellipse class="' + headClass + '" cx="' + x + '" cy="' + y + '" rx="6" ry="4.5" transform="rotate(-15 ' + x + " " + y + ')"/>';
    });
    ...
    if (copyBtn) {
      copyBtn.addEventListener("click", function () {
        ...
        navigator.clipboard.writeText(el.value).then(function () {
          ...
          setTimeout(function () {
            copyBtn.textContent = "Copy for Stradella";
          }, 1500);
        });
      });
    }
