...
with sync_playwright() as playwright:
    ...
    try:
        for width in (320, 390, 768, 1440):
            ...
            page.locator('#bp-prison-blues [data-bp-step="1"]').first.click()
            original = draft(page)
            page.locator("#bp-rest-mode").click()
            rest = draft(page)
            check(rest["steps"][1] == {"ticks": 6, "presses": []}, "Rest converts the selected chord immediately and retains its duration")
            check(rest["steps"][:1] + rest["steps"][2:] == original["steps"][:1] + original["steps"][2:],
                  "Note/rest conversion leaves neighboring steps unchanged")
            page.locator("#bp-note-mode").click()
            check(draft(page) == original, "Note restores the last selected chord, spelling, and label")
            page.locator("#bp-undo").click()
            check(draft(page) == rest, "Conversion is one undo step")
            page.locator("#bp-redo").click()
            check(draft(page) == original, "Redo restores the converted chord")
            page.locator('[data-bp-slot="1"]').focus()
            page.keyboard.press("r")
            check(draft(page) == rest, "R changes a selected chord to a rest")
            page.keyboard.press("r")
            check(draft(page) == original, "R restores the selected chord without changing rhythm")
            page.locator('#bp-prison-blues [data-bp-step="2"]').first.click()
            before_rest = draft(page)
            page.locator("#bp-note-mode").click()
            check(draft(page)["steps"][2]["notes"] == [{"midi": 48, "name": "C"}]
                  and draft(page)["steps"][2]["ticks"] == 6, "Note directly fills an existing rest with the remembered pitch")
            page.locator("#bp-undo").click()
            check(draft(page) == before_rest, "Undo restores the original imported rest")
            click_note(page, 2, 24, touch=width < 400)
            check(draft(page)["steps"][2]["notes"] == [{"midi": 53, "name": "F"}], "A single tap on a rest enters the clicked pitch")
            page.locator("#bp-rest-mode").click()
            page.locator('[data-bp-slot="2"]').focus()
            page.keyboard.press("g")
            check(draft(page)["steps"][2]["notes"] == [{"midi": 55, "name": "G"}]
                  and page.locator("#bp-note-mode").get_attribute("aria-pressed") == "true", "Typing a pitch exits Rest mode and replaces the rest")
            # Button-based patterns keep their assignments through a quick toggle.
            page.locator('[data-pattern-id="example:0"]').get_by_role("button", name="Edit", exact=True).click()
            buttons = draft(page)
            page.locator("#bp-rest-mode").click()
            page.locator("#bp-note-mode").click()
            check(draft(page) == buttons, "Note/rest round trip preserves legacy button assignments")
