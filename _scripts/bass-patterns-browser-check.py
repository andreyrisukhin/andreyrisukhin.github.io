...
parser.add_argument("--url", default="http://127.0.0.1:4182/music/bass-patterns/")
...
with sync_playwright() as p:
    ...
    try:
        for width in (320, 390, 768, 1440):
            ...
            page.goto(args.url)
            check(page.locator(".bp-card").count() == 3, f"Three example basslines at {width}px")
            check(page.locator(".bp-card svg").count() == 3, "Each bassline has its own staff")
            ...
            check(page.locator(".bp-card").count() == 4, "Save adds a separate bassline card")
            ...
            page.locator("#bp-save").click()
            check(page.locator(".bp-card").count() == 5, "Multiple saved basslines coexist")
            ...
            page.reload()
            check(page.locator(".bp-card").count() == 5, "Saved basslines survive reload")
            ...
            page.locator(".bp-card").first.get_by_role("button", name="Edit", exact=True).click()
            page.locator("#bp-score [data-bp-step='1']").first.click()
            check(page.locator("#bp-step-title").inner_text() == "Step 2", "Clicking staff notation selects that step")
            context.close()
        ...
        card = page.locator(".bp-card").first
        ...
        card.get_by_role("button", name="Play", exact=True).click()
        page.locator(".bp-card").nth(1).get_by_role("button", name="Play", exact=True).click()
        check(page.get_by_role("button", name="Stop", exact=True).count() == 1, "Only one bassline plays at a time")
        page.locator(".bp-card").nth(1).get_by_role("button", name="Stop", exact=True).click()
        ...
        page.reload()
        check(page.locator(".bp-card svg").count() == 3, "Collection works after offline reload")
