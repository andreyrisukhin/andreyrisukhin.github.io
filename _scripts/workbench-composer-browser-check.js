...
(async () => {
  ...
  if (window.workbenchCheckStage === "reload") {
    assert(names() === "Am7,D7,C/E,Gmaj7", "Reload restores edited progression");
    ...
    assert($("chord-name").textContent === "C/E" && midis() === "64,72,67", "Restored hand inspector uses picked note order and octave");
    ...
    assert(names() === "Am7,D7,C/E,Gmaj7", "View switch preserves recovered progression");
    click("#remove");
    assert(names() === "Am7,D7,Gmaj7", "Recovered progression remains editable");
    $("wb-saved-drawer").open = true;
    click("[data-load]");
    assert(names() === "Am7,D7,C/E,Gmaj7" && midis() === "64,72,67", "Named saved practice restores exact ordered notes");
    assert(
  ...
  click('[data-gap="2"]');
  click("#commit");
  assert(names() === "Am7,D7,C/E,Gmaj7", "Picked chord enters the actual progression");
  assert($("chord-name").textContent === "C/E" && midis() === "64,72,67", "Insertion updates the shared staff and hand inspector");
  ...
  click('[data-view="progression"]');
  assert(names() === "Am7,D7,C/E,Gmaj7", "Chord edits do not replace progression");
  click("#undo");
  assert(names() === "Am7,D7,Gmaj7", "Progression undo history survives independent chord edits");
  click("#redo");
  assert(names() === "Am7,D7,C/E,Gmaj7", "Redo restores picked chord after view switch");
