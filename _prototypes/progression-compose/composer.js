/* Standalone chooser. All sources prepare candidates; only commit changes harmony. */
(function () {
  "use strict";
  ...
  function renderCandidate() {
    ...
    if (candidate.length && before) {
      ...
      $("candidate-relation").textContent =
        relation.movement + ". Shared pitch classes: " + (relation.common.join(" · ") || "none") + ". This does not choose a voice-leading pattern.";
    }
  ...
  function renderNotes() {
    ...
    if (model) {
      window.PrototypeChordInspector.renderNotation($("picked-staff"), model, {
        voices,
  ...
  function updatePicked(next, focusId, message) {
    ...
    if (focusId) ($(focusId) || document.querySelector("[data-pc]")).focus({ preventScroll: true });
    announce(message);
  }
  ...
  function render() {
    ...
    if (focused.disabled || !focused.isConnected) {
      const target =
        step !== undefined
          ? document.querySelector('[data-step="' + doc.state.selected + '"]')
          : gap !== undefined
            ? document.querySelector('[data-gap="' + doc.state.gap + '"]')
            : $("tab-" + source);
  ...
  async function hear(chords, { progression = false, midis = null, labels = null, pickedNotes = false } = {}) {
    ...
    const result = Player.play(sounds, {
      ...
      onStep: (index) => {
        document.querySelectorAll(".is-sounding").forEach((el) => el.classList.remove("is-sounding"));
        if (progression) document.querySelector('[data-step="' + index + '"]')?.classList.add("is-sounding");
        if (pickedNotes)
          $("picked-staff")
            .querySelectorAll("[data-midi]")
            .forEach((note) => {
  ...
  document.querySelector("main").addEventListener("click", (event) => {
    ...
    if (el.dataset.step !== undefined) {
      ...
      document.querySelector('[data-pc="' + pc + '"]').focus({ preventScroll: true });
  ...
  $("hear-progression").addEventListener("click", () => hear(doc.state.chords, { progression: true }));
