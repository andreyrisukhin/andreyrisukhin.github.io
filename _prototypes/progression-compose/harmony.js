...
window.ComposerHarmony = (function () {
  ...
  function voices(chord) {
    const voiced = M.voices(chord);
    if (chord.voicing !== "bass-octave") return voiced;
    return voiced.map((voice) => {
      const midi = 60 + voice.pc + (voice.pc < chord.notes[0] ? 12 : 0);
      return { ...voice, midi, octave: voice.octave + (midi - voice.midi) / 12 };
    });
  }
